...
public class BankAccountService {
    ...
    @Autowired
    public BankAccountService(BankAccountRepository bankAccountRepository, TransactionRepository transactionRepository, UserService userService) {
    ...
    public boolean depositAmount(BankAccountBindingModel model){
        ...
        else if(BigDecimal.ZERO.compareTo(model.getAmount()) > 0) return false;
...
//        return this.bankAccountRepository.saveAndFlush(bankAccount) != null;
    ...
    public boolean withdrawAmount(BankAccountBindingModel model){
        ...
        else if(bankAccount.getBalance().compareTo(model.getAmount()) < 0) return false;
...
//        return this.bankAccountRepository.saveAndFlush(bankAccount) != null;
    ...
    public boolean transferAmount(BankAccountBindingModel model){
        ...
        BankAccount toAccount = this.bankAccountRepository
                .findById(model.getReceiverId())
                .orElse(null);

        if(fromAccount == null || toAccount == null) return false;
        else if(BigDecimal.ZERO.compareTo(model.getAmount()) > 0) return false;

        if(fromAccount.getBalance().compareTo(model.getAmount()) < 0) return false;
        ...
        return
                this.bankAccountRepository.saveAndFlush(fromAccount) != null &&
