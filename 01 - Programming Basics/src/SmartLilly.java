...
//   получава играчки, а за всеки четен (2, 4, 6...n) получава пари. За втория рожден ден получава 10.00 лв,
...
// Ако парите на Лили са достатъчни:      o “Yes! {N}” - където N е остатъка пари след покупката
// Ако парите не са достатъчни:          o “No! {М}“ - където M е сумата, която не достига
// Числата N и M трябва да за форматирани до вторият знак след десетичната запетая.
        ...
        Double priceWashingMachine = Double.valueOf(reader.readLine());
        Integer priceToy = Integer.parseInt(reader.readLine());

        Integer moneyTakenFromBrother = 0;
        Double savings = 0.0;
        Double moneyGivenForCurrentBirthday = 0.0;
        Integer toys = 0;
        for (int i = 1; i <= age; i++) {
            if(i % 2 != 0){
                toys++;
            }else{
                moneyGivenForCurrentBirthday += 10;
                savings += moneyGivenForCurrentBirthday;
                moneyTakenFromBrother++;
            }
        }
        Double N = savings + priceToy * toys - moneyTakenFromBrother - priceWashingMachine;

        if(N >= 0){
            System.out.printf("Yes! %.2f", N);
        }else {
            System.out.printf("No! %.2f", Math.abs(N));
        }
